...
struct ImuMeasurement {
  std::array<double, 3>
      linear_accel{}; ///< Linear acceleration (ax, ay, az) in m/s^2
  std::array<double, 3>
      angular_vel{}; ///< Angular velocity (gx, gy, gz) in rad/s
  std::array<double, 4>
      orientation{}; ///< Optional pre-computed orientation (quaternion)
...
struct Measurement {
  ...
  std::optional<std::vector<std::vector<double>>>
      H;                                ///< Observation model matrix (Custom)
  std::optional<std::vector<double>> z; ///< Measurement vector (Custom)
  std::optional<std::vector<std::vector<double>>>
      R; ///< Measurement noise covariance (Custom)
};
...
/**
 * @brief Configuration for Complementary Filter.
 */
struct ComplementaryConfig {
  double alpha = 0.98; ///< Weighting factor (0.0 - 1.0)
};
 ...
 */
struct EkfConfig {
  std::vector<double> initial_process_noise; ///< Diagonal elements for Q
  std::vector<double>
      initial_measurement_noise; ///< Diagonal elements for R (default/fallback)
};

/**
 * @brief Unified Configuration for FusionCore.
 */
struct FusionConfig {
  FilterType filter_type = FilterType::COMPLEMENTARY;
  int state_dim = 6;

  ComplementaryConfig complementary_config;
...
public:
  ...
  FusionCore(FilterType filter_type = FilterType::COMPLEMENTARY,
             int state_dim = 6, const State *initial_state = nullptr);
   ...
   * @param config Configuration parameters for the fusion engine.
   */
  void initialize(const FusionConfig &config);
